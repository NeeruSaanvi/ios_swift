...
class AttendeeHistoryVC: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        table?.register(UINib(nibName: "EventCell", bundle: nil), forCellReuseIdentifier: "EventCell")
        
        self.title = "Historial de asistente"
    ...
    func getEventList()
    {
        ...
        APIModal().myEvent(type: "A", time: "past", target: self, action: #selector(getPastEventListHandle(response:)))
        
        APIModal().myEvent(type: "A", time: "future", target: self, action: #selector(getFutureEventListHandle(response:)))
...
//        }
        ...
        var interval: NSMutableAttributedString!
            ...
            if(segment.selectedSegmentIndex == 0)
            {
                interval = comman().daysBetweenDates(startDate: Date(), endDate: earlierDate!, type:"attendee")
            }
            else
            {
